...
if (!empty($id)) {
    // Edit
    $param['id'] = $id;
    $data = Api::call(Configure::read('API.url_projects_detail'), $param);
...
// Create breadcrumb
$listPageUrl = h($this->BASE_URL . '/projects');
$this->Breadcrumb->setTitle($pageTitle)
        ->add(array(
            'link' => $listPageUrl,
            'name' => __('LABEL_PROJECT_LIST'),
        ))
        ->add(array(
            'name' => $pageTitle,
        ));
...
if ($this->request->is('post')) {
    ...
    if ($form->validate($data)) {
        // Call API
        $id = Api::call(Configure::read('API.url_projects_addupdate'), $data);
        $err = Api::getError();
