...
if (!empty($data)) {
    foreach ($data as $val) {
        ...
        $totalLack += $val['lack'];
        $totalOriginPrice += $val['total_origin_price'];
    }
    if ($type == 2) {
        ...
        foreach ($data as $val) {
            ...
            $revenueByCustomer[$val['customer_id']]['total_origin_price'] += $val['total_origin_price'];
        }
        ...
        foreach ($data as $val) {
            ...
            foreach ($products as $p) {
                ...
                $revenueByProduct[$p['id']]['origin_price'] += !empty($p['origin_price']) ? $p['origin_price'] * $p['qty'] : 0;
            }
        }
...
$profit = $this->dataPagination($data, $page, $limit);
